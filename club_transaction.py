                    ...
                    if club_type is not None:
                        if club_type == 'department':
                            if department_input and club_name_input:
                                query = "SELECT * FROM department_club WHERE club_name=%s and department=%s"
                                cursor.execute(query, (club_name_input,department_input))
                                ...
                                query = "SELECT * FROM department_club WHERE department=%s"
                                cursor.execute(query, (department_input,))
                            elif club_name_input:
                                query = "SELECT * FROM department_club WHERE club_name=%s"
                                cursor.execute(query, (club_name_input,))
                                updated_rows = cursor.fetchall()
                                df_updated = pd.DataFrame(updated_rows, columns=[i[0] for i in cursor.description])
                                num_rows_updated = len(df_updated)
                                query = "SELECT * FROM department_club WHERE department=%s"
                        ...
                        try:
                            if club_name_input:
                                ...
                                df_updated = pd.DataFrame(dept_rows, columns=[i[0] for i in cursor.description])
                                num_rows_updated = len(df_updated)

                                query_club = "SELECT * FROM club WHERE club_name=%s"
                                cursor.execute(query_club, (club_name_input,))
                                club_rows = cursor.fetchall()
                                df_updated = pd.DataFrame(club_rows, columns=[i[0] for i in cursor.description])
                                num_rows_updated = len(df_updated)
                ...
                if conn.is_connected():
                    ...
                    if submit_button and club_type is not None:
                        ...
                        if filter_field_input and filter_data_input:
                            if club_type == 'department':
                                query=f"SELECT * FROM department_club WHERE {filter_field_input} = %s"
                                ...
                                query=f"SELECT * FROM club WHERE {filter_field_input} = %s and club_type=%s"
                                cursor.execute(query,(filter_data_input,club_type))
                                updated_rows = cursor.fetchall()
                                df_updated = pd.DataFrame(updated_rows, columns=[i[0] for i in cursor.description])
                                # Count the number of rows in the DataFrame
                                num_rows_updated = len(df_updated)
                                query = f"update club set {field_input}=%s where {filter_field_input}=%s and club_type=%s"
                                cursor.execute(query, (data_input, filter_data_input,club_type))
                            if cursor.rowcount == 0:
                                ...
                                if auto_commit or commit_changes:
                                    ...
                                    st.success("Changes will be rollbacked. Check 'Commit Changes' to commit.")
                        else:
                            if club_type == 'department':
                                query="SELECT * FROM department_club"
                                cursor.execute(query,())
                                ...
                                query="SELECT * FROM club where club_type=%s"
                                cursor.execute(query,(club_type,))
                                updated_rows = cursor.fetchall()
                                # Store the updated data in a DataFrame
                                df_updated = pd.DataFrame(updated_rows, columns=[i[0] for i in cursor.description])
                ...
                if conn.is_connected():
                    ...
                    if submit_button and club_type is not None:
                        ...
                        if field_input and data_input:
                            if club_type == 'department':
                                query=f"SELECT * FROM department_club WHERE {field_input}=%s"
                                cursor.execute(query,(data_input,))
                                updated_rows = cursor.fetchall()
                                df_updated = pd.DataFrame(updated_rows, columns=[i[0] for i in cursor.description])
                                num_rows_updated = len(df_updated)
                                ...
                                query=f"SELECT * FROM club WHERE {field_input}=%s and club_type=%s"
                                cursor.execute(query,(data_input,club_type))
                                updated_rows = cursor.fetchall()
                                df_updated = pd.DataFrame(updated_rows, columns=[i[0] for i in cursor.description])
                                num_rows_updated = len(df_updated)
                            ...
                            if club_type == 'department':
                                query=f"SELECT * FROM department_club"
                                cursor.execute(query,())
                                updated_rows = cursor.fetchall()
                                df_updated = pd.DataFrame(updated_rows, columns=[i[0] for i in cursor.description])
                                num_rows_updated = len(df_updated)
                                query = f"delete from department_club"
                                cursor.execute(query, (data_input,))
                            elif club_type in ['cultural', 'technical']:
                                query=f"SELECT * FROM club WHERE club_type=%s"
                                cursor.execute(query,(club_type,))
