        ...
        if club_type is not None:
            ...
            try:
                ...
                if conn.is_connected():
                    ...
                    # Check if the form is submitted
                    if club_type == 'department':
                        query = "insert into department_club(Department,faculty_advisor,club_name,club_email,club_poc,poc_netid) values(%s,%s,%s,%s,%s,%s)"
                        cursor.execute(query, (department_input, faculty_name_input, club_name_input, club_email_input, club_poc_input, poc_netid_input))
                    elif club_type in ['cultural', 'technical']:
                ...
                if submit_button and club_type is not None:
                    ...
                    if filter_field_input and filter_data_input:
                        ...
                        if club_type == 'department':
                            ...
                            query = f"update club set {field_input}=%s where club_type=%s"
                            cursor.execute(query, (data_input, club_type))
                        if cursor.rowcount == 0:
                            st.error("No such record exists.")

                    restore = st.form_submit_button("Restore Changes")
                    save = st.form_submit_button("Save Changes")
                    if restore:
                        conn.rollback()
                        st.success("Changes rollbacked! ")
                    elif save:
                        conn.commit()
                ...
                if submit_button and club_type is not None:
                    ...
                    if field_input and data_input:
                        if club_type == 'department':
                            ...
                            query = f"delete from club where {field_input}=%s and club_type= %s "
                            cursor.execute(query, (data_input, club_type))
                        if cursor.rowcount == 0:
                            st.error("No such record exists.")
                    else:
                        if club_type == 'department':
                            ...
                            query = "delete from club where club_type= %s "
                            cursor.execute(query, (club_type,))
                        if cursor.rowcount == 0:
                            st.error("No such record exists.")
